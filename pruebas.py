...
for prueba in range(1000):
    ...
    if len(mov) != len(monedas):
        cierran = False
    if suma_sofia + suma_mateo != sum(monedas):
        cierran = False
...
suma_sofia, suma_mateo, mov, m_sofia, m_mateo = juego_monedas([10, 2, 5, 20, 50])
chequear("[10,2,5,20,50] da 75 a 12 con monedas [50,20,5]",
         suma_sofia == 75 and suma_mateo == 12 and m_sofia == [50, 20, 5])


# Casos borde.
suma_sofia, suma_mateo, mov, m_sofia, m_mateo = juego_monedas([42])
chequear("una sola moneda: se la lleva Sophia",
         suma_sofia == 42 and suma_mateo == 0)

suma_sofia, suma_mateo, mov, m_sofia, m_mateo = juego_monedas([3, 8])
chequear("dos monedas: Sophia se queda la mayor",
         suma_sofia == 8 and suma_mateo == 3)

suma_sofia, suma_mateo, mov, m_sofia, m_mateo = juego_monedas([7, 7, 7, 7])
chequear("todas iguales y n par: empata", suma_sofia == suma_mateo)

suma_sofia, suma_mateo, mov, m_sofia, m_mateo = juego_monedas([7, 7, 7])
chequear("todas iguales y n impar: gana Sophia", suma_sofia > suma_mateo)
